 ...
 * uses (see NexusHouses' own PulseBridge, copied here). The moment CrafterCraftListener upgrades a
 * forged item, it reports that as "forge.craft" progress -- NexusCraftersGuild works exactly the
 * same with or without NexusEndeavors installed at all; report() is a silent no-op either way.
 */
final class EndeavorsBridge {
    ...
    private void warnOnce(Exception e) {
        ...
        warned = true;
        plugin.getLogger().warning("[NexusCraftersGuild] NexusEndeavors is installed, but couldn't accept a "
